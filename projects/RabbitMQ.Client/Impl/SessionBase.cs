...
namespace RabbitMQ.Client.Impl
{
    internal abstract class SessionBase : ISession
    {
        ...
        public ValueTask TransmitAsync<TMethod, THeader>(in TMethod cmd, in THeader header, IReadOnlyMemoryOwner<byte> body, CancellationToken cancellationToken = default)
            where TMethod : struct, IOutgoingAmqpMethod
            where THeader : IAmqpHeader
        {
            if (!IsOpen && cmd.ProtocolCommandId != ProtocolCommandId.ChannelCloseOk)
